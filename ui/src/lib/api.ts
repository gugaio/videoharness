...
const ManifestReportContentBaseSchema = z.object({
  ...
  ai: z.object({
    ...
    promptAudits: z.array(z.object({ agentId: z.enum(["timeline-playback", "container-encoding", "manifest-delivery", "abr-switch-investigator", "lead-investigator"]), attempt: z.number().int().positive(), state: z.enum(["completed", "failed"]), provider: z.string(), model: z.string(), systemPrompt: z.string(), prompt: z.string(), toolNames: z.array(z.string()), toolCalls: z.array(z.object({ name: z.string(), input: z.string(), output: z.string() })), packetMetrics: z.object({ packetBytes: z.number().int().nonnegative(), evidenceIdCount: z.number().int().nonnegative(), sharedEvidenceIdCount: z.number().int().nonnegative(), sharedEvidenceRatio: z.number().min(0).max(1) }).optional(), output: z.unknown().optional() })).optional(),
  }).optional(),
});
