...
import type { PostgresRecordingDeletion } from "../adapters/postgres-recording-deletion.js";
...
describe("deleteRecording", () => {
  it("removes published and temporary files before deleting the database row", async () => {
    const calls: string[] = [];
    const repository = { canDelete: vi.fn(async () => "ready"), delete: vi.fn(async () => { calls.push("database"); return "deleted" as const; }) } as unknown as PostgresRecordingDeletion;
  ...
  it("does not touch storage for an experiment recording", async () => {
    const repository = { canDelete: vi.fn(async () => "linked_to_experiment") } as unknown as PostgresRecordingDeletion;
