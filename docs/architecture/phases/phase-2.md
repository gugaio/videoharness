...
- [x] Tres posicoes deterministicas por playlist: inicio, meio e fim (modo
  `sample`); o default `full` coleta uma janela contigua de ate 60s por variant,
  centrada no horario de incidente relatado quando existir.
...
- [x] Coleta de janela candidata DASH distribuida pela ladder e audio, usando
  resolucoes/direcao apenas quando explicitamente relatadas pelo usuario.
- [x] Inspecao local fMP4 de `moof`/`tfdt`/`tfhd`/`trun`/`mdat` e de `hvcC`, com
  PTS/DTS, flags, IRAP inicial e hashes de parameter sets.
- [x] `AbrAssessment` protocol-neutral em toda investigation HLS/DASH, com
  baseline de ladder, cobertura, verdict, transicoes disponiveis e medicoes
  recomendadas.
- [x] Selecao DASH de representations baseada no relato quando explicito ou em
  amostragem distribuida da ladder; nenhuma resolucao fixa faz parte da regra.
...
- O modo de coleta e configurável por `VIDEO_HARNESS_MEDIA_SAMPLE_MODE`:
  `full` (padrao) coleta uma janela contigua de ate
  `VIDEO_HARNESS_MEDIA_SAMPLE_MAX_SECONDS` (padrao 60s) por variant, centrada no
  horario de incidente relatado quando existir (sem horario, a janela parte do
  inicio); `sample` baixa apenas inicio/meio/fim.
- A selecao usa o tempo declarado no manifest (duracao dos segmentos HLS;
  `presentationStart/EndSeconds` no DASH). Os limites de bytes sao redes de
  seguranca: cada resposta e limitada a 128 MiB por padrao e a soma da amostra a
  512 MiB; ambos sao configuraveis pelo worker.
...
- Suportado: HLS MPEG-TS clear e DASH static fMP4 clear dentro dos limites
  documentados, com amostragem limitada e avaliacao da ladder completa declarada.
- Fora do MVP: byte ranges, DRM/decryption, LL-HLS, MPD dinamico e comprovacao de
  decode/render em device.
...
- A matriz estrutural compara containers, init segments, timestamps e IRAP nas
  transicoes selecionadas. Resultado de decode/render em hardware exige
...
- Pendencias: `SegmentBase`/`sidx`, MPD dinamico completo, byte ranges, DRM
  decriptado, downloads repetidos por POP e reproducao em hardware real.
